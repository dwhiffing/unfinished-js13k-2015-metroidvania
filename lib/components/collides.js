...
export function getColliders(one) {
  ...
  entities.get().forEach((two) => {
    if (shouldCollide(one, two) && _checkCollision(one, two)) {
      colliders.push(two)
    }
  })
  return colliders
...
export function shouldCollide(one, two) {
  if (one === two || !one.collides || !two.collides) return false
  if (!one.collides.collidesWith || one.collides.collidesWith.indexOf(two.collides.index) === -1) return false
  return true
}

export function getColliderDirections(one) {
  let colliders = {
    left: null,
    right: null,
    bottom: null,
